					...
					<div class="card shadow">
						...
						<div class="card-header d-flex p-0">
							<h3 class="card-title p-3">
								<button class="btn btn-sm btn-primary" id="tambah_data_user"><i class="fa fa-plus"></i>&nbsp;&nbsp;Tambah Data</button>
					...
					<div class="modal fade" id="modal-tambah" tabindex="-1" role="dialog"
						...
						<div class="modal-dialog modal-lg modal-dialog-centered" role="document">
							<div class="modal-content">
								<form id="form-tambah" action="<?= base_url('user/tambah_data_user'); ?>" method="POST"
									...
									<div class="modal-body">
											...
											<div class="col-sm-7">
												<select name="pegawai" id="pegawai" class="form-control">
                                                    ...
                                                    <?php foreach($list_pegawai as $lp): ?>
                                                        <option value="<?= $lp->id_pegawai;?>"><?= $lp->nama_pegawai;?></option>
                                                    <?php endforeach;?>
                                                </select>
											</div>
										</div>
                                        ...
                                        <div class="form-group d-flex justify-content-center">
											<label for="inputEmail3" class="col-sm-3 control-label">Passrword</label>
											<div class="col-sm-7">
...
<script>
	var table = $("#tbl-data").DataTable({
			...
			'ajax': {
				'url': "<?php echo base_url('pengguna/get_user');?>",
				'type': 'POST',
				'dataType': 'json',
    ...
    function edit(i){
        $.ajax({
            url:"<?= base_url('pengguna/show_user'); ?>",
            type:"POST",
            dataType:"JSON",
            data:{id_user:i},
            success:function(res){
                console.log(res.id_pegawai);
	...
	$(document).ready(function () {
		...
		$('#tambah_data_user').click(function () {

			$.ajax({
				url 	: "<?= base_url('pengguna/tampil_form_tambah') ?>",
				type 	: "POST",
				dataType: "JSON",
		...
		$("#form-tambah").submit(function(e){
			...
			$.ajax({
				url:"<?= base_url('pengguna/tambah_data_user');?>",
				type:"POST",
				dataType:"JSON",
				data:data,
				success:function(res){
		...
		$("#form-edit").submit(function(e){
			...
			$.ajax({
				url:"<?= base_url('pengguna/ubah_data_user')?>",
				data:data,
				type:"POST",
				dataType:"JSON",
				success:function(res){
...
function aktif(i){
        $.ajax({
            url:"<?= base_url('pengguna/user_aktif'); ?>",
            type:"POST",
            dataType:"JSON",
            data:{id_user:i},
            success:function(res){
                if(res.status == "ok"){
                    Swal.fire({
                        title: 'Sukses !',
                        text: 'Data User dan Pegawai berhasil di aktivasi',
                        type: 'success',
                    });
                    table.ajax.reload(null, false);
                }
                    ...
                    Swal.fire({
                        title: 'Maaf !',
                        text: 'Data User dan Pegawai gagal di aktivasi',
                        type: 'error',
                    });
                }
    ...
    function nonaktif(i){
        $.ajax({
            url:"<?= base_url('pengguna/user_non_aktif'); ?>",
            type:"POST",
            dataType:"JSON",
            data:{id_user:i},
            success:function(res){
                if(res.status == "ok"){
                    Swal.fire({
                        title: 'Sukses !',
                        text: 'Data user dan pegawai berhasil di non aktifkan',
                        type: 'success',
                    });
                    table.ajax.reload(null, false);
                }
                    ...
                    Swal.fire({
                        title: 'Maaf !',
                        text: 'Data user dan pegawai gagal di non aktifkan',
                        type: 'error',
                    });
                }
